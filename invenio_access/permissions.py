...
class Permission(_Permission):
    ...
    def _split_actionsneeds(needs):
        ...
        for need in needs:
            action_needs.add(need) if need.method == "action" else \
    ...
    def _load_permissions(self):
        ...
        # split ActionNeeds and any other *Needs in separates Sets
        ...
        # "allow_by_default = False" means that when needs/excludes are empty,
        # then it should deny access.
        # By default, `flask_principal.Permission.allows` will allow access
        # if needs/excludes are empty!
        needs_empty = len(result.needs) == 0 or len(result.excludes) == 0
        ...
        if needs_empty and deny_access_when_empty_needs:
            ...
            result.excludes.update(action_excludes)
